...
class UsersController extends Controller {
    ...
    public function edituser( $id ) {
        ...
        $getdistrict = DB::table( 'district' )->orderBy( 'id', 'Asc' )->get();
        $org = DB::table( 'organization' )->orderBy( 'id', 'Asc' )->get();

        return view( 'users/edituser', compact( 'editusers', 'usertypes', 'getdistrict', 'org' ) );
    }

    public function profile() {
        $usersid =  Auth::user()->id;
        $editusers = DB::table( 'users' )->where( 'id', '=', $usersid )->first();
        $usertypes = DB::table( 'user_type' )->orderBy( 'id', 'Asc' )->get();
        $getdistrict = DB::table( 'district' )->orderBy( 'id', 'Asc' )->get();
        $org = DB::table( 'organization' )->orderBy( 'id', 'Asc' )->get();

        return view( 'users/profile', compact( 'editusers', 'usertypes', 'getdistrict', 'org' ) );
    }
    ...
    public function updateusers( Request $request ) {
        ...
        $aadhar = '';
        if ( $request->aadharfile != null ) {
            $aadhar = $userid.'.'.$request->file( 'aadharfile' )->extension();

            $filepath = public_path( 'upload'.DIRECTORY_SEPARATOR.'aadhar_image'.DIRECTORY_SEPARATOR );
            move_uploaded_file( $_FILES[ 'aadharfile' ][ 'tmp_name' ], $filepath.$aadhar );
            $sql = "update users set aadharfile='$aadhar' where id = $userid";
            DB::update( DB::raw( $sql ) );
        }
        $pro = '';
        if ( $request->profile != null ) {
            $pro = $userid.'.'.$request->file( 'aadharfile' )->extension();

            $filepath = public_path( 'upload'.DIRECTORY_SEPARATOR.'profile_image'.DIRECTORY_SEPARATOR );
            move_uploaded_file( $_FILES[ 'profile' ][ 'tmp_name' ], $filepath.$pro );
            $sql = "update users set profile='$pro' where id = $userid";
            DB::update( DB::raw( $sql ) );
        }
